...
class Add_Menu_Form(FlaskForm):
      ...
      cuisine = StringField("Enter Cuisine(Optional): ", validators=[DataRequired(), Length(min=3,max=50)])
      category = StringField("Enter Cateogry of Item(Optional): ", validators=[DataRequired(), Length(min=3,max=50)])
      type = StringField("Enter Type of Item(Optional)", validators=[DataRequired(), Length(min=3,max=50)])
      prep_time = IntegerField("Enter Prep Time of Item(Minutes): ", validators=[DataRequired()])
      submit = SubmitField("Update")

      def validate_name(self,name):
          obj=db.session.scalar(sa.select(Menu).where(name.data==Menu.name and  Menu.vendor_id == current_user.id ))
          if obj is not None:
